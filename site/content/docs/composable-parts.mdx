...
import { ComposableDemo } from "@/components/demos/lazy";
...
- **Composable mode** — set `layout="composable"` and build the layout yourself using `Sheet.*` parts

<ComposableDemo />

## Enabling composable mode

Pass `layout="composable"` to the provider. This disables the auto header and scroll wrapper, giving your content full control of the panel layout:
