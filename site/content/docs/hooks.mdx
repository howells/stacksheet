...
import { NavigateDemo } from "@/components/demos/lazy";
...
This is useful for tab-like navigation within a single sheet where you want instant content changes without slide animations.
...
Returns the current panel's context. Must be called inside a component rendered by the sheet stack (works in both classic and [composable mode](/docs/composable-parts)).
...
This is the hook that powers the `Sheet.*` composable parts. Use it when you need panel-level context for custom controls. See [Composable Parts](/docs/composable-parts) for the full guide.
