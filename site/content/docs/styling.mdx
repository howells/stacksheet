...
Stacksheet uses Tailwind CSS classes for default styling. Make sure `tailwindcss >= 4.0.0` is installed as a peer dependency, and configure your Tailwind content paths to include the library:

```css
/* app.css */
@import "tailwindcss";
@source "../node_modules/@howells/stacksheet/dist";
```

## Class names

Stacksheet renders with sensible Tailwind defaults (white background, subtle shadow, backdrop overlay). Override with CSS classes:
...
| `--muted-foreground` | `rgba(0,0,0,0.25)` | `Sheet.Handle` bar color |
