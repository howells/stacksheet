...
On Chromium 120+ browsers, Stacksheet registers a [CloseWatcher](https://developer.mozilla.org/en-US/docs/Web/API/CloseWatcher) to handle the Android system back gesture. This lets users swipe back to pop or close sheets, matching native Android behavior.
...
Per-sheet override via action options:

```tsx
// Component-direct pattern
open(UserProfile, { userId: "abc" }, { ariaLabel: "User profile for John" });

// Type registry pattern
open("user-profile", "user-1", {
  userId: "abc",
}, { ariaLabel: "User profile for John" });
```
...
In [composable mode](/docs/composable-parts) (`layout="composable"`), the panel uses `aria-labelledby` and `aria-describedby` instead of `aria-label`. These are automatically linked to `Sheet.Title` and `Sheet.Description` via matching IDs.
